...
class AttackConsole:
    ...
    def attack_1_rogue_device(self):
        ...
        # We try to send data using an AID that the Gateway doesn't know
        payload = "AID_UNKNOWN_DEVICE|Temp:Critical|SN:1|TS:0"
        # We sign it with our rogue key (valid crypto, but unauthorized identity)
        sig = sign(payload, ROGUE_SK)
        
        packet = {"payload": payload, "sig": sig}
        self.client.publish("telemetry/UNKNOWN", json.dumps(packet))
        print(">> 📤 Packet Sent. Check Gateway for 'ROGUE DEVICE BLOCKED'.")
